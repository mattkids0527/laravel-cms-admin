...
- 遇到架構決策（如：新模組、Repository interface 設計、Event 設計），先說明方案與理由，確認使用者同意後再實作
...
## 專案技術棧
- 請閱讀 `vibe-coding/skills/` 底下的相關檔案中的規範進行開發
- 執行時會先告知所使用的 skills 檔案有哪些並且依照其規範執行
- 若有不建議或是不適用該規範可以回覆給使用者討論
